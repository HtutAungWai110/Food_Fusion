...
function DownloadableCard({ recipe }) {
  const {
  ...
  const handleDownload = () => {
    ...
    link.href = card_path;
    link.download = `${title.replace(/\s+/g, '-').toLowerCase()}-card.pdf`;
    link.target = '_blank';
    ...
    >
      <Card className="w-full max-w-md overflow-hidden flex flex-col h-full border-none shadow-lg bg-card/50 backdrop-blur-sm group">
        ...
        <CardFooter className="pt-2">
          ...
          <Button onClick={handleDownload} className="w-full bg-orange-500 hover:bg-orange-600 text-white font-bold transition-all">
            <Download/>
            Download
          </Button>
        </CardFooter>
...
export default memo(DownloadableCard);